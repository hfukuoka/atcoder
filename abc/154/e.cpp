...
int main(){
    ...
    cin >> n >> K;
    int m = n.size();
    vector dp(m+1, vector(K+1, vll(2, 0)));
    dp[0][0][0] = 1;
    rep(i, m){
        rep(j,K+1){
            rep(k, 2){
                if(k==1){
                    dp[i+1][j][1] += dp[i][j][1];
                    if(j+1<=K)dp[i+1][j+1][1] += dp[i][j][1]*9;
                }else{ // 現在の桁までnと一致
                    if(n[i]-'0'==0){
                        dp[i+1][j][0] += dp[i][j][0]; // nの次の桁が0のとき、0しか取れず、それはnと一致している
                    }else{
                        dp[i+1][j][1] += dp[i][j][0]; // 次の数字が0ならn以下
                        if(j+1<=K)for(int c=1; c<(n[i]-'0'); ++c){
                            dp[i+1][j+1][1] += dp[i][j][0]; //次の数字がn[i]以下ならn以下で、0以外の数字の個数が増える
                        }
