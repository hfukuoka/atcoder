...
long long solve(long long start, long long flag){
    long long res = INF;
    long long crr;
    if(flag==1){
        auto it = lower_bound(all(t), start);
        if(it!=t.end()){
            crr = *it - start;
            chmin(res, crr);
        }
    ...
    if(flag==2){
        auto it = lower_bound(all(s), start);
        if(it!=s.end()){
            crr = *it-start;
            chmin(res, crr);
        }
