...
int main(){
    ...
    vll a(n);
    rep(i, n)cin >> a[i];
    lazy_segtree<S, op, e, F, mapping, composition, id> lazy_segtree(a);
    int q;
    cin >> q;
    rep(qi, q){
        int t;
        ...
        if(t==1){
            ll x;
            cin >> x;
            lazy_segtree.apply(0, n, {0, x});
        }
        if(t==2){
            int i, x;
            cin >> i >> x;
            i--;
            ll px = lazy_segtree.get(i);
            lazy_segtree.set(i, px+x);
        }
        if(t==3){
            int i;
            cin >> i;
            i--;
            cout << lazy_segtree.get(i) << endl;
        }
    }
    return 0;
}