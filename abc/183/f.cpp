...
int main(){
    ...
    rep(qi, q){
        ...
        if(t==1){
            ...
            a = uf.leader(a); // 代表元同士の話に置き換える
            b = uf.leader(b);
            int l = uf.merge(a, b); // unionfindではサイズが小さい方を大きい方につなげ、大きい方の代表元が新たな代表元となる
            if(l!=a)swap(a, b); // aが新たな代表元となるようにする。bのグループををaのグループに加えるようにする。その結果、小さいbのグループがが大きいaのグループに加えられる。
            for(auto [k, v]:mps[b]) mps[a][k] += v;
        }else if(t==2){
            int x, y;
            cin >> x >> y;
            x--, y--;
