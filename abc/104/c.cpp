...
int main(){
    ...
    vvll dp(d+1, vll(sum+1, -1));
    dp[0][0] = 0;
    rep(i, d){
        rep(j, sum+1){
            if(dp[i][j]==-1)continue; // ありえない遷移は飛ばす
            rep(k, p[i]+1){ // i点の問題をk個選ぶ
                if(j+k>sum)break;
                if(k==p[i])chmax(dp[i+1][j+k], dp[i][j]+k*(i+1)+c[i]);
                if(k<p[i])chmax(dp[i+1][j+k], dp[i][j]+k*(i+1));
            }
        }
    }
    int ans = sum;
