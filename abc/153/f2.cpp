...
int main(){
    ...
    int r = 0; // 半開区間[l, r)
    rep(l, n){
        if(l>0)buf[l] += buf[l-1]; // imos法で累積和
        auto [x, h] = ps[l];
        if(buf[l]>=h)continue; // すでに体力がゼロなら無視
        chmax(r, l+1); // もし右端が左にきてたら右にずらす
        while(r<n and ps[r].first <= x + 2*d) r++;
        ll rem = h - buf[l]; // 残りの体力
        ll cnt = (rem + a - 1) / a; // 爆破回数
        ans += cnt;
        buf[l] += cnt * a; // imos法で差分を記録
        buf[r] -= cnt * a;
    }
    cout << ans << endl;
    return 0;
