...
/* topo_sort(G): グラフG をトポロジカルソート
    ...
    計算量: O(|E|+|V|)
 */
void dfs(const Graph1 &G, int v, vector<bool> &used, vector<int> &ans) {
    used[v] = true;
    for (auto nv : G[v]) {
        if (!used[nv]) {
            dfs(G, nv, used, ans);
...
vector<int> topo_sort(const Graph1 &G) {  // bfs
    vector<int> ans;
    int n = (int)G.size();
...
int main(){
    ...
    rep(i, n){
        for(auto [to, cost]:G[i]){
            if(dis[i]+1==dis[to])g[i].push_back(to);
        }
    }
    vector<int> v = topo_sort(g);
    vll dp(n, 0);
    dp[a] = 1;
    for(auto x:v){
        for(auto y:g[x]){
