...
long long dfs(long long c, vector<long long> a){
    if(c==-1)return 0;
    vector<long long> S, T;
    for(int i=0; i<a.size(); ++i){
        if((a[i]>>c)&1)S.push_back(a[i]);
        else T.push_back(a[i]);
    }
    if(S.size()==0)return dfs(c-1, T);
    if(T.size()==0)return dfs(c-1, S);
    return min(dfs(c-1, S), dfs(c-1, T)) | (1<<c);
}
