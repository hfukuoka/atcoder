...
double solve(int x, int y, int z){
    ...
    dp[x][y][z]=double(n)/(x+y+z);
    if(x-1>=0)dp[x][y][z] += solve(x-1, y, z)*double(x)/(x+y+z);
    if(x+1<=n && y-1>=0)dp[x][y][z] += solve(x+1, y-1, z)*double(y)/(x+y+z);
    if(y+1<=n && z-1>=0)dp[x][y][z] += solve(x, y+1, z-1)*double(z)/(x+y+z);
    return dp[x][y][z];
}
