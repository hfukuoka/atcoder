...
int main(){
    ...
    vi n(m);
    rep(i, m)n[i]=s[i]-'0';
    vector dp(m+1, vector(2, vector<mint>(d, 0))); // dp[i][j][k]: i桁目まで決めて、j=0:same, j=1:smaller、各桁の和をdで割ったあまりがkのときの個数
    dp[0][0][0]=1;
    rep(i, m)rep(j, 2)rep(k, d){
        rep(nd, 10){
            int ni=i+1, nj=j, nk=(k+nd)%d;
            if(j==0){
                if(nd<n[i])nj=1;
                if(nd>n[i])continue;
            }
